...
class Component_ToonChain(Component_Base):
	...
	def create_bone_infos(self):
		...
		self.make_def_chain(
			...
			,preserve_volume = self.params.chain.preserve_volume
			,create_ctr_def_ctrls = self.params.chain.unlock_deform
		)
	...
	def get_relink_target(self, org_i, con) -> BoneInfo:
		...
		if 'TAIL' in con.name:
			if len(self.main_str_bones) <= org_i + 1:
				...
				self.raise_metarig_error(f'Cannot move constraint "{con.name}" from "{org_bone.name}" to final STR bone since it does not exist! Make sure "Tip Control" param is enabled!')
	...
	def make_main_str_bone(self
		...
		if org_bone.prev and self.params.chain.align_roll and not self.params.chain.sharp and num_segments > 1:
	...
	def connect_parent_chain_rig(self):
		...
		parent_rig.setup_deform_bone(
			...
			,preserve_volume = parent_rig.params.chain.preserve_volume
		)
