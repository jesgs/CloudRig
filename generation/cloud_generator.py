...
class CloudRig_Generator:
    ...
    def raise_generation_error(self
            ,description_short = "Generation Error"
            ,description = ""
            ,**kwargs
        ):
    ...
    def instantiate_rig_components(self) -> Dict[str, Component_Base]:
        ...
        for pb in component_bones_ordered:
            ...
            if parent_component_rna:
                ...
                assert parent_instance, "Error: Parent should've been instantiated already! Are we not looping hierarchically?"
    ...
    def create_test_animation(self):
        ...
        if not any(
                [
                    hasattr(rig.params.fk_chain, 'test_animation_generate') and 
                    rig.params.fk_chain.test_animation_generate 
    ...
    def get_symmetry_rig_component(self, component: "Component_Base") -> "Component_Base":
    ...
    def components_write_pbone_data(self, target_rig):
        for bone_info in self.bone_infos:
            ...
            if not pose_bone.use_custom_shape_bone_size and bone_info.use_custom_shape_bbone_scaling:
                pose_bone.custom_shape_scale_xyz *= bone_info.bbone_width * 10 * self.scale
