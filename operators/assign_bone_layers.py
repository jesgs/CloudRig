...
class CLOUDRIG_OT_layer_assign(Operator):
    ...
    def update_layers(self, context):
        ...
        bones = (
            ...
            else [pb.bone for pb in context.selected_pose_bones]
        )
        for bone in bones:
    ...
    layers: BoolVectorProperty(
        size=32,
        subtype='LAYER',
        description=f"Layers to assign selected bones to",
    ...
    def draw(self, context):
        ...
        if not rig:
            rig = context.active_object
        draw_layers_ui(
            self.layout, rig, show_hidden_checkbox=True, layer_prop_owner=self
        )
...
registry = [CLOUDRIG_OT_layer_assign, CLOUDRIG_OT_layer_init]


def register():
    register_hotkey(
        CLOUDRIG_OT_layer_assign.bl_idname,
