...
from bpy.types import Context, Event, Menu, Operator, PoseBone, UILayout
...
class POSE_OT_reload_selected_custom_shape(Operator):
    ...
    @staticmethod
    def get_bones_to_reload(context: Context):
        """Yield pose bones whose custom shape can be reloaded from a CloudRig widget library."""
        if not context.selected_pose_bones:
            return
        for pb in context.selected_pose_bones:
            if (
...
class POSE_OT_copy_custom_shape_to_selected_bones(Operator):
    ...
    def execute(self, context: Context):
        ...
        for i, pb in enumerate(context.selected_pose_bones):
            ...
            pb.use_transform_at_custom_shape = active_pb.use_transform_at_custom_shape
            pb.use_transform_around_custom_shape = active_pb.use_transform_around_custom_shape
...
class POSE_OT_edit_widget_of_selected_bones(Operator):
    ...
    @classmethod
    def poll_without_linked_check(cls, context: Context):
    ...
    def enter_custom_shape_edit_mode(self, context: Context):
        """Reveal widget objects, transform them to match their bones, then enter mesh edit mode."""
...
class MESH_OT_return_to_pose_mode(Operator):
    ...
    @staticmethod
    def restore_all_widgets_visibility(context: Context):
        """Restore visibility of all widget objects that were revealed during shape editing."""
        global WIDGETS_VISIBILITY

        if WIDGETS_VISIBILITY:
            for visibility in WIDGETS_VISIBILITY[:]:
                try:
...
class POSE_OT_assign_selected_object_as_custom_shape(Operator):
    ...
    def execute(self, context: Context):
        shape = next(o for o in context.selected_objects if o.type == 'MESH')
        counter = 0
        for pb in context.selected_pose_bones:
...
class POSE_OT_edit_bone_display_props(Operator, BONE_PT_display):
    ...
    def draw_cloudrig_settings(self, context: Context, layout: UILayout, pbone: PoseBone):
    ...
    def draw_bone_color_settings(self, layout: UILayout, pbone: PoseBone):
