...
from bpy.types import Context, Event, Image, Object, Operator, SpaceView3D
...
def get_3d_view(context: Context) -> SpaceView3D | None:
    """Return the active SpaceView3D, preferring the current area if it is a 3D view."""
...
@contextmanager
def selection_state(context: Context, active_obj: Object | None = None, selected_obs: list[Object] | None = None):
    """Context manager that temporarily sets the active object and selection, then restores them."""
...
@contextmanager
def focus_objects(context: Context, objects: list[Object] | None = None, margin=0.1):
    """Context manager that fits the 3D view to the given objects, then restores the view matrix."""
    view_3d = get_3d_view(context)
...
def focus_view_on_objects(context: Context, objects: list[Object] | None = None, margin=0.1):
    if not objects:
...
@contextmanager
def active_camera(context: Context, camera: Object):
...
def fit_view3d_to_coords(context: Context, center: Vector, coords: list[Vector]):
    view_3d = get_3d_view(context)
    if not view_3d:
        return
    region_3d = view_3d.region_3d
...
def get_world_bounding_box(obj: Object) -> list[Vector]:
...
def render_widget(
    ...
    *,
    ...
    """Render a viewport thumbnail for a widget object, optionally saving it to disk."""
    with (
        viewport_settings(context, light='FLAT', color_type='SINGLE', single_color=color, resolution=render_resolution),
        widget_geonodes([obj], thickness=thickness),
        focus_objects(context, [obj], margin=0.2) if focus_view else nullcontext(),
    ):
        bpy.ops.render.opengl()
        redraw_viewport()
        render_result = bpy.data.images.get('Render Result')
        if save:
            full_path = get_thumbnail_path(obj)

            img_processor = ImageProcessor(render_result, widget_name(obj.name))
...
def get_thumbnail_path(obj: Object) -> Path:
...
def draw_menu(self, _context: Context):
