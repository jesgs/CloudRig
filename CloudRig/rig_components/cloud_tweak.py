    ...
    def base__load_metarig_bones(self) -> dict[str, BoneInfo]:
        ...
        if len(bone_infos) > 1:
            self.add_log(
                ...
                description=rpt_(
                    "The Bone Tweak component type will only affect the first bone of the chain. "
    ...
    def create_component_interactions(self, context: Context):
        ...
        if not self.bone_to_tweak:
            self.add_log(
                ...
                description=rpt_(
                    'Could not find a bone called "{bone}" on the Target Rig. '
        ...
        if self.params.tweak.shape:
            ...
            bone_to_tweak.use_transform_at_custom_shape = meta_pbone.use_transform_at_custom_shape
            bone_to_tweak.use_transform_around_custom_shape = meta_pbone.use_transform_around_custom_shape
