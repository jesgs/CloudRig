...
class Component_Curve_Hooked(Component_Base):
    ...
    def __get_mirror_point(
        ...
        if offset > threshold:
            ...
            self.raise_generation_error(
                description=(
                    f'The nearest point to the X-axis flipped coordinate of point '
                    f'"{point_name} ({curve.path_resolve(point_path).co.xyz})" is point '
                    f'"{opp_point_name} (({curve.path_resolve(opp_point_path).co.xyz}))".'
                    f'\nDistance: {offset}\nThreshold: {threshold}\n'
    ...
    def define_bone_sets(cls):
        ...
        cls.define_bone_set('Curve Root', color_palette='THEME02', wire_width=2.0)
        cls.define_bone_set('Spline Roots', color_palette='THEME12', wire_width=2.0)
        cls.define_bone_set('Curve Hooks', color_palette='THEME01', wire_width=1.5)
        cls.define_bone_set('Curve Handles', color_palette='THEME09', wire_width=1.0)
