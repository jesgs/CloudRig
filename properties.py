...
class BoneSets(PropertyGroup):
    ...
    def make_bone_set_property_groups() -> Dict[str, type]:
        ...
        for rig_type_name, rig_module in rigs.rig_types.items():
            if not hasattr(rig_module, 'bone_set_definitions'):
                continue

            for bone_set_name, bone_set_definition in rig_module.bone_set_definitions.items():
...
registry = list(get_param_classes().values()) + list(BoneSets.bone_set_property_groups.values()) + [
    BoneSets,
    RigParams,
    RigElement,
    RigElementBone,
